...
import {BuiltinGrammar, Grammar, GrammarMatcher, TokenizerInfo} from '..';
...
async function getTokenizerInfoFromUrl(
  ...
  // 3. Decode
  const decodedVocab =
      await TokenizerInfo.createTokenizerInfo(encodedVocab, vocabType, prependSpace);
  return decodedVocab;
}

/**
 * Identical to `test_to_json_roundtrip()` in `test_grammar_parser.py`.
 * Tests all APIs exposed in the class Grammar
 */
...
d_1 ::= ("" | ("d"))
`;
    const grammar1 = await Grammar.createGrammar(before)
    const serializedStr1 = grammar1.serialize();
    const grammar2 = await Grammar.deserialize(serializedStr1);
...
describe('Test all BuiltinGrammar APIs', () => {
  const ebnf_grammar =
      String.raw`basic_escape ::= ["\\/bfnrt] | "u" [A-Fa-f0-9] [A-Fa-f0-9] [A-Fa-f0-9] [A-Fa-f0-9]
...
`
  ...
  const schema =
      String
          .raw`{"properties": {"num": {"default": 0, "title": "Num", "type": "integer"}, "opt_bool": {"anyOf": [{"type": "boolean"}, {"type": "null"}], "default": null, "title": "Opt Bool"}, "size": {"anyOf": [{"type": "number"}, {"type": "null"}], "title": "Size"}, "name": {"default": "", "title": "Name", "type": "string"}}, "required": ["size"], "title": "MainModel", "type": "object"}`;
...
describe('Test TokenizerInfo', () => {
  test('Test basic tokenizer info', async () => {
    ...
    const tokenizerInfo =
        await TokenizerInfo.createTokenizerInfo(dummyVocab, dummyVocabType, false);
...
describe('Test GrammarMatcher E2E', () => {
  ...
  test('test_token_operations', async () => {
    ...
    const tokenizerInfo = await TokenizerInfo.createTokenizerInfo(vocab, 'byte_level', false);
    ...
    const expected = [
      ...
      ['}', ', ', '\n', ' '],
      ['</s>'],
    ] const result: Array<Array<string>> = [] for (let i = 0; i <= input_ids.length; i++) {
      const input_id = input_ids[i];
      // Find rejected IDs
      const bitmask = await matcher.getNextTokenBitmask();
      const rejectedIDs =
          await GrammarMatcher.debugGetMaskedTokensFromBitmask(bitmask, matcher.getVocabSize());
  ...
  test('test reset and termination', async () => {
    ...
    const tokenizerInfo = await TokenizerInfo.createTokenizerInfo(vocab, 'byte_level', false);
    const matcher = await GrammarMatcher.createGrammarMatcher(
        jsonGrammar, tokenizerInfo, undefined, undefined, undefined, 5
    );
    tokenizerInfo.dispose();
...
`;
    const vocab = ['a', 'bb'];
    const grammar = await Grammar.createGrammar(grammar_ebnf);
    const tokenizerInfo = await TokenizerInfo.createTokenizerInfo(vocab, 'byte_level', false);
...
describe('Test json schema E2E', () => {
  // Equivalent to MainModel used in the python test, except removed minItem and maxItem from
  // tuple_field to avoid long log
  const schemaStr =
      String
          .raw`{"properties": {"integer_field": {"title": "Integer Field", "type": "integer"}, "number_field": {"title": "Number Field", "type": "number"}, "boolean_field": {"title": "Boolean Field", "type": "boolean"}, "any_array_field": {"items": {}, "title": "Any Array Field", "type": "array"}, "array_field": {"items": {"type": "string"}, "title": "Array Field", "type": "array"}, "tuple_field": {"prefixItems": [{"type": "string"}, {"type": "integer"}, {"items": {"type": "string"}, "type": "array"}], "title": "Tuple Field", "type": "array"}, "object_field": {"additionalProperties": {"type": "integer"}, "title": "Object Field", "type": "object"}, "nested_object_field": {"additionalProperties": {"additionalProperties": {"type": "integer"}, "type": "object"}, "title": "Nested Object Field", "type": "object"}}, "required": ["integer_field", "number_field", "boolean_field", "any_array_field", "array_field", "tuple_field", "object_field", "nested_object_field"], "title": "MainModel", "type": "object"}`;
  ...
  test('Test with Llama3.2, byte_level', async () => {
    ...
    // 3. Decode
    const tokenizerInfo =
        await TokenizerInfo.createTokenizerInfo(encodedVocab, 'byte_level', false);
    ...
    const final_rejected_tokens =
        (await GrammarMatcher.debugGetMaskedTokensFromBitmask(final_bitmask, matcher.getVocabSize())
        );
  ...
  test('Test with Phi-3.5, byte_fallback, _acceptString', async () => {
    ...
    // 3. Decode
    const tokenizerInfo =
        await TokenizerInfo.createTokenizerInfo(encodedVocab, 'byte_fallback', false);

    // 4. Instantiate matcher; note that phi-3.5 has 32064 as vocab size in `config.json`
    const grammar = await BuiltinGrammar.jsonSchema(schemaStr, 2);
    const matcher =
        await GrammarMatcher.createGrammarMatcher(grammar, tokenizerInfo, undefined, false, 32064);
    tokenizerInfo.dispose();
