...
from .apply_token_bitmask_cpu import apply_token_bitmask_inplace_cpu
...
def apply_token_bitmask_inplace(
    ...
    *,
    ...
    If the logits is on CUDA, we launch a CUDA kernel to apply bitmask. If the logits is
    on CPU, we use a CPU implementation that is functionally equivalent.

    Parameters
    ----------
    logits : torch.Tensor
        The tensor to apply the bitmask to. Should be on either CPU or CUDA.

    bitmask : torch.Tensor
        The bitmask to apply. Should be generated by allocate_token_bitmask and
        filled by fill_next_token_bitmask. The first dimension is `batch_size` if `indices` is
        not specified. Otherwise it is `len(indices)`.
